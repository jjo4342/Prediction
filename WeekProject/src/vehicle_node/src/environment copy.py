...
class Environments(object):
    ...
    def run(self):
        ...
        for id_ in self.vehicles.keys():
            if id_ == 0:  # SDV (Self Driving Vehicle)
                sensor_info = self.vehicles[id_].get_measure(self.vehicles)
                for info in sensor_info:
                    other_id = info[0]
                    if self.check_collision_at_intersection(id_, other_id):
                        self.vehicles[id_].step_manual(ax=0, steer=0)  # 충돌 위험이 있으므로 정지
                        # rospy.loginfo(f"Collision detected between vehicle {id_} and {other_id} \n")
                        continue
                    self.vehicles[id_].step_manual(ax=0.1, steer=0)  
                # 주변 환경을 평가하여 적절한 조치 취하기
    ...
    def check_collision_at_intersection(self, ego_id, other_id):
        # 교차점에서 충돌을 체크
        if ego_id in self.int_pt_list and other_id in self.int_pt_list[ego_id]:
            for inter, ego_idx, other_idx in self.int_pt_list[ego_id][other_id]:
                if abs(self.vehicles[ego_id].s - self.vehicles[other_id].s) < 0.5:
                    return True
        return False
