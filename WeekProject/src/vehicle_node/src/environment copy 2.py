...
class Environments(object):
    ...
    def predict_path(self, vehicle, sensor_info, possible_lanes):
        # 경로 예측 로직
        # 예를 들어, 감지된 자동차의 속도와 현재 차선을 바탕으로 다음 차선을 예측
        if sensor_info['vx'] > 0:  # 속도가 양수이면 다음 차선을 선택
            return possible_lanes[0] if possible_lanes else None  # 가능한 차선이 있으면 첫 번째 차선 반환
        else:
            return possible_lanes[-1] if possible_lanes else None  # 가능한 차선이 있으면 마지막 차선 반환
